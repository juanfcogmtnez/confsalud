...
for i in range(len(df)):
    ...
    if str(df.loc[i][2]) != grupo3:
        ...
        grupo = grupo+1
        grupo3id = grupo
        dict = {'id':grupo,'parent':grupo2id,"text":'<span id="ltitle" style="color:grey">Grupo: </span><span id="title" style="color:black;font-weight:bold">'+str(df.loc[i][2])+'</span><span id="lqty" style="color:grey"> &nbsp;| cantidad: </span><span id="qty" style="color:black;font-weight:bold">1</span><span id="lsup" style="color:grey">&nbsp;| m2: </span><span id="sup" style="color:black;font-weight:bold">'+str(df.iloc[i][5])+'</span>'}
        file.append(dict)
    grupo = grupo+1
    dict = {'id':grupo,'parent':grupo3id,"text":'<span id="ltitle" style="color:grey">Artículo: </span><span id="title" style="color:black;font-weight:bold">'+str(df.iloc[i][8])+'</span><span id="lqty" style="color:grey"> &nbsp;| cantidad: </span><span id="qty" style="color:black;font-weight:bold">'+str(df.iloc[i][10])+'</span><span id="lsup" style="color:grey">&nbsp;| Tipo: </span><span id="sup" style="color:black;font-weight:bold">'+str(df.iloc[i][9])+'</span>'}
    file.append(dict)

print(file)

with open("static/json/cnd.json", "w") as outfile:
